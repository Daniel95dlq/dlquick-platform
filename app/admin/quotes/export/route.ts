...
export async function GET(req: Request) {
  ...
  const rows = await (prisma as any).quoteRequest.findMany({
    ...
    skip,
    where,
    select: { id: true, createdAt: true, serviceTitle: true, name: true, email: true, phone: true, status: true },
  })
  const header = ['id','createdAt','serviceTitle','name','email','phone','status']
  const body = rows.map((r: any) => [r.id, r.createdAt.toISOString(), r.serviceTitle, r.name, r.email, r.phone, r.status]
